...
public class CollectTreeTest {
	...
	public void simple_test() {
		...
		composite.add(new MatcherToResult<>(whatIf(string -> string.equals("hello"), string -> "world")));
		composite.add(new MatcherToResult<>(whatIf(string -> string.equals("!"), string -> "null")));
	...
	private CollectTree.Root<String, IStringScanner> chain(BinaryOperator<String> converter) {
		CollectTree<String, IStringScanner> chain = new CollectTree<>(
				whatIf(scanner -> scanner.existsIf(Character::isAlphabetic), IStringScanner::nextAlpha)) ;
		chain.addChild(new CollectTree<>(
				whatIf(scanner -> scanner.exists("("),
